...
EventAction::EndOfEventAction(const G4Event* anEvent)
{
...

  {
    ...
    if (id >= 0 && id_edep >= 0) {
      ...
      G4double pmtpc[MaxHitsTPC]       = {0.0};
      G4int    qqtpc[MaxHitsTPC]       = {0};
      G4double pxtpc[MaxHitsTPC]       = {0.0};
      G4double pytpc[MaxHitsTPC]       = {0.0};
      G4double pztpc[MaxHitsTPC]       = {0.0};
      G4double pptpc[MaxHitsTPC]       = {0.0};
      G4double vtxxtpc[MaxHitsTPC]     = {0.0};
      G4double vtxytpc[MaxHitsTPC]     = {0.0};
      G4double vtxztpc[MaxHitsTPC]     = {0.0};
      G4double vtxpxtpc[MaxHitsTPC]    = {0.0};
      G4double vtxpytpc[MaxHitsTPC]    = {0.0};
      G4double vtxpztpc[MaxHitsTPC]    = {0.0};
      // G4double vtxpptpc[MaxHitsTPC]  = {0.0};
      G4double vtxenetpc[MaxHitsTPC]   = {0.0};
      G4double detpc[MaxHitsTPC]       = {0.0};
      G4int    laytpc[MaxHitsTPC]      = {0};
      G4double lentpc[MaxHitsTPC]      = {0.0};
      G4int    nparticle               = 0;
      ...
      std::vector<std::vector<G4double>> remain_edep;
      std::vector<std::vector<G4int>> remain_ilay;
      std::vector<G4int> remain_pid;
      std::vector<G4int> remain_tid;
      std::vector<G4int> remain_parentid;
      std::vector<G4int> remain_parentpid;
      ...
      if (pad_configure == 4) {
        // --- Edep: group hits by tid, average pos / sum edep per layer ---
        std::map<G4int, std::size_t> tid_to_remain_idx;
        std::vector<std::vector<G4double>> open_posx;
        std::vector<std::vector<G4double>> open_posy;
        std::vector<std::vector<G4double>> open_posz;
        std::vector<std::vector<G4double>> open_edep;
        ...
        // --- Child → parent edep add (same ilay, dist < 30 mm) ---
        {
        ...
        // --- Pad → Edep lookup indexes (after merge) ---
      ...
      for (G4int i = 0; i < nhits; ++i) {
        ...
        if (pad_configure == 3) {
          ...
          if (edep_configure == 0)
            ...
            if (i == 0) {
              ...
              if (std::fabs((xyz - xyz_pre).mag()) > 30) {
                G4ThreeVector mom_pre = (*HC)[i+1]->GetMomentum();
                G4double beta_pre = (*HC)[i+1]->GetBeta();
                G4double E_pre = mom_pre.mag() / beta_pre;
                G4double E_post = mom.mag() / beta;
                ...
                G4cout << mom_pre.mag() << " , " << mom.mag()
                       << ", diff : " << mom_pre.mag()-mom.mag() << G4endl;
              }
            }
          }
#endif
        ...
        if (tid_it != tid_to_slot.end()) {
          ...
          // vtxpptpc[slot] = std::hypot(vtxmom[0], vtxmom[1], vtxmom[2]);
        ...
        if (slot >= 0 && ilay > -1) { //-->  -1 : TPC, layer is from 0 to 38. 2012.10.30
          ...
          case 0:
            gAnaMan.SetCounterDataSimple(slot, tof, xyz, mom, tid, pid, ilay,
                                         irow, beta, edep/CLHEP::MeV,
                                         parentid, parentpid, charge,
                                         tlength, slength);
            break;
          case 1:
          ...
          default:
            gAnaMan.SetCounterDataSimple(slot, tof, xyz, mom, tid, pid, ilay,
                                         ...
                                         parentid, parentpid, charge,
                                         tlength, slength);
            G4cout << "TPC Resolution type is not determined. Now using constant resolution"
                   << G4endl;
            break;
          }
        }
      ...
      for (G4int i = 0; i < nparticle; i++) {
        gAnaMan.SetTPCData(i, pidtr[i], ptidtpc[i], ptidtpc_pid[i],
                           pxtpc[i], pytpc[i], pztpc[i], pptpc[i],
                           qqtpc[i], pmtpc[i], detpc[i], lentpc[i], laytpc[i],
...
G4double
EventAction::CalculateAverage(const std::vector<G4double>& pos)
{
  if (pos.size() == 0) return -1000.;

  G4double pos_sum = 0.0;
...
G4double
EventAction::CalculateSum(const std::vector<G4double>& edep)
{
  G4double edep_sum = 0.0;

  for (std::size_t i = 0; i < edep.size(); ++i) {
    edep_sum += edep[i];
  }
