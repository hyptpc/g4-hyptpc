...
class BDCHit : public G4VHit {
  ...
  public:
    BDCHit();
    //  BDCHit(G4ThreeVector& axyz, G4double t);
    BDCHit(G4ThreeVector& axyz, G4ThreeVector& apxyz, G4double t);
    BDCHit(G4ThreeVector& axyz, G4ThreeVector& apxyz, G4double t,
    ...
    BDCHit(const BDCHit& right);
    const BDCHit& operator=(const BDCHit& right);
...
// inline functions
// ====================================================================
  inline BDCHit::BDCHit(const BDCHit& right)
: G4VHit()
{
  xyz= right.xyz;
  pxyz= right.pxyz;
  ...
  inline const BDCHit& BDCHit::operator=
(const BDCHit& right)
{
  xyz= right.xyz;
  pxyz= right.pxyz;
...
typedef G4THitsCollection<BDCHit> BDCHitsCollection;
extern G4Allocator<BDCHit> BDCHitAllocator;

inline void* BDCHit::operator new(size_t)
{
  return static_cast<void *>( BDCHitAllocator.MallocSingle() );

}

inline void BDCHit::operator delete(void* aHit)
{
  BDCHitAllocator.FreeSingle ( static_cast<BDCHit*> (aHit) );
}
