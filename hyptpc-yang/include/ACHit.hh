...
class ACHit : public G4VHit {
  ...
  public:
    ACHit();
    //  ACHit(G4ThreeVector& axyz, G4double t);
    ACHit(G4ThreeVector& axyz, G4ThreeVector& apxyz, G4double t);
    ACHit(G4ThreeVector& axyz, G4ThreeVector& apxyz, G4double t,
    ...
    ACHit(const ACHit& right);
    const ACHit& operator=(const ACHit& right);
...
// inline functions
// ====================================================================
  inline ACHit::ACHit(const ACHit& right)
: G4VHit()
{
  xyz= right.xyz;
  pxyz= right.pxyz;
  ...
  inline const ACHit& ACHit::operator=
(const ACHit& right)
{
  xyz= right.xyz;
  pxyz= right.pxyz;
...
{
  void* aHit= (void*)ACHitAllocator.MallocSingle();
  return aHit;
}

inline void ACHit::operator delete(void* aHit)
{
  ACHitAllocator.FreeSingle((ACHit*) aHit);
}
