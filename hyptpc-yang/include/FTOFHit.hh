...
class FTOFHit : public G4VHit {
  ...
  public:
    FTOFHit();
    //  FTOFHit(G4ThreeVector& axyz, G4double t);
    FTOFHit(G4ThreeVector& axyz, G4ThreeVector& apxyz, G4double t);
    FTOFHit(G4ThreeVector& axyz, G4ThreeVector& apxyz, G4double t,
...
// inline functions
// ====================================================================
  inline FTOFHit::FTOFHit(const FTOFHit& right)
: G4VHit()
{
  xyz= right.xyz;
  pxyz= right.pxyz;
  ...
  inline const FTOFHit& FTOFHit::operator=
(const FTOFHit& right)
{
  xyz= right.xyz;
  pxyz= right.pxyz;
...
typedef G4THitsCollection<FTOFHit> FTOFHitsCollection;
extern G4Allocator<FTOFHit> FTOFHitAllocator;

inline void* FTOFHit::operator new(size_t)
{
  void* aHit= (void*)FTOFHitAllocator.MallocSingle();
  return aHit;
}

inline void FTOFHit::operator delete(void* aHit)
{
  FTOFHitAllocator.FreeSingle((FTOFHit*) aHit);
}
